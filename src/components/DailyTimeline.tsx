...
const DailyTimeline: React.FC<DailyTimelineProps> = ({
  ...
  return (
    <div className="relative w-full overflow-y-auto h-[calc(100vh-180px)] pr-2">
          ...
          {timeSlots.map((slot, index) => (
            <div 
              ...
              className={`
                ...
                ${slot.isHalfHour ? 'text-xs text-gray-400' : 'text-sm font-medium text-gray-700 dark:text-gray-300'}
              `}
            >
              {slot.time}
            </div>
          ...
          {timeSlots.map((slot, index) => (
            <div 
              ...
              className={`
                ...
                ${slot.isHalfHour ? '' : 'border-t-2'}
                ${currentTimeIndex === index ? 'bg-yellow-50/50 dark:bg-yellow-900/10' : ''}
              `}
            >
              ...
              {blocksByTimeIndex[index]?.map((block) => (
                ...
                >
                  {block.isRoutine ? (
                    ...
                    <div 
                      className={`
                        rounded-md p-2 shadow-sm cursor-pointer transition-all hover:shadow-md
                        ${block.color || 'bg-gray-100 dark:bg-gray-800'}
                      `}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {block.data.focus && (
                          <span className="text-red-500 dark:text-red-400 text-xs px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 rounded-full">
                            Priorytet
                          </span>
                        )}
                      </div>
